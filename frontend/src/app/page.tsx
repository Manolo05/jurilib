    ...
    <>
      ...
      <section className="relative min-h-[85vh] flex items-center overflow-hidden noise-overlay">
        ...
        <div className="relative z-10 mx-auto max-w-6xl px-4 py-20 md:py-28">
          ...
          <form
            action="/recherche"
            className="mt-10 bg-juri-card rounded-2xl border border-juri-border p-2 flex flex-col md:flex-row gap-2 max-w-3xl"
          >
            <select
              name="specialty"
              className="flex-1 px-4 py-3.5 rounded-xl bg-juri-bg border border-juri-border text-juri-text font-sans text-sm focus:outline-none focus:border-gold/50 transition-all cursor-pointer"
              defaultValue=""
            >
            ...
            <select
              name="city"
              className="flex-1 px-4 py-3.5 rounded-xl bg-juri-bg border border-juri-border text-juri-text font-sans text-sm focus:outline-none focus:border-gold/50 transition-all cursor-pointer md:border-l"
              defaultValue=""
            >
          ...
          <div className="mt-6 flex flex-wrap gap-2 text-sm">
            {SPECIALTY_PILLS.map((s) => (
              <Link
                key={s}
                href={`/droit/${s}`}
                className="rounded-full bg-juri-card border border-juri-border px-3 py-1 text-juri-muted hover:border-gold/30 hover:text-gold transition-all font-sans text-xs"
              >
      ...
      <section className="mx-auto max-w-6xl px-4 py-20 grid md:grid-cols-3 gap-6">
        {[
          ...
          <div
            key={title}
            className="rounded-2xl border border-juri-border bg-juri-card p-6 hover:border-gold/30 hover:bg-juri-card-hover transition-all duration-300 group"
          >
      ...
      {top.length > 0 && (
        <section className="mx-auto max-w-6xl px-4 pb-20">
          <div className="flex items-end justify-between">
            <h2 className="text-2xl font-serif font-medium text-juri-text">
              Avocats les mieux notés
            </h2>
            <Link href="/recherche" className="text-sm text-gold hover:text-gold-light transition-colors font-sans">
              Voir tout le réseau →
            </Link>
          </div>
          <div className="mt-6 grid md:grid-cols-3 gap-5">
            {top.map((l) => (
              <Link key={l.id} href={`/avocat/${l.slug}`}
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {l.specialties.slice(0, 2).map((s) => (
                    <span key={s.specialty.slug} className="text-xs bg-gold/10 text-gold border border-gold/20 rounded-full px-2 py-0.5 font-sans">
                      {s.specialty.label}
                    </span>
                  ))}
                </div>
                <p className="mt-4 font-serif font-semibold text-gold text-right">{l.consultationPrice} €</p>
              </Link>
            ))}
          </div>
        </section>
      ...
      {cities.length > 0 && (
        <section className="mx-auto max-w-6xl px-4 pb-20">
          ...
          <p className="mt-2 text-juri-muted font-sans">
            Disponibilités en cabinet ou en visio dans les plus grandes villes françaises.
          </p>
          <div className="mt-6 flex flex-wrap gap-3">
            {cities.map((c) => (
              <Link key={c} href={`/ville/${c.toLowerCase()}`}
                className="rounded-xl border border-juri-border bg-juri-card px-4 py-2 hover:border-gold/30 hover:bg-juri-card-hover transition-all font-sans text-sm text-juri-muted hover:text-gold">
                Avocat à <strong className="text-juri-text">{c}</strong>
              </Link>
            ))}
          </div>
        </section>
      ...
      <section className="border-t border-juri-border bg-juri-card/50">
        <div className="mx-auto max-w-6xl px-4 py-16 md:flex md:items-center md:justify-between gap-8">
          <div>
            ...
            <p className="mt-2 text-juri-muted max-w-xl font-sans leading-relaxed">
              Rejoignez la plateforme et développez votre cabinet&nbsp;: agenda intégré,
              visibilité locale, commission uniquement sur rendez-vous honorés.
            </p>
          </div>
          <Link href="/avocat-inscription"
            className="mt-6 md:mt-0 inline-block rounded-xl bg-gradient-to-r from-gold-dark via-gold to-gold-light text-juri-bg px-8 py-3.5 hover:shadow-[0_0_30px_rgba(201,168,76,0.3)] font-sans font-semibold transition-all">
            Rejoindre le réseau
          </Link>
        </div>
      </section>
          ...
          <div className="mt-6 grid md:grid-cols-3 gap-5">
            {top.map((l) => (
              ...
              >
                ...
                <p className="mt-4 font-serif font-semibold text-gold text-right">
                  {l.consultationPrice} \u20ac
                </p>
              </Link>
            ))}
